...
int VideoPlayer::initSwr()
{
    ...
    _aSwrInSpec.chnLayout  = _aDecodeCtx->channel_layout;
    _aSwrInSpec.chs        = _aDecodeCtx->channels;

    _aSwrOutSpec.sampleFmt  = AV_SAMPLE_FMT_S16;
    _aSwrOutSpec.sampleRate = 44100;
    _aSwrOutSpec.chnLayout  = AV_CH_LAYOUT_STEREO;
    _aSwrOutSpec.chs        = av_get_channel_layout_nb_channels(_aSwrOutSpec.chnLayout);
    _aSwrOutSpec.bytesPerSampleFrame =
        _aSwrOutSpec.chs * av_get_bytes_per_sample(_aSwrOutSpec.sampleFmt);

    _aSwrCtx = swr_alloc_set_opts(nullptr,
                                  _aSwrOutSpec.chnLayout,
                                  _aSwrOutSpec.sampleFmt,
                                  _aSwrOutSpec.sampleRate,
                                  _aSwrInSpec.chnLayout,
...
void VideoPlayer::sdlAudioCallback(Uint8* stream, int len)
{
    ...
    while (len > 0) {
        ...
        int filLen = _aSwrOutSize - _aSwrOutIdx;
        filLen     = std::min(filLen, len);
        int volumn = _mute ? 0 : ((_volumn * 1.0 / Max) * SDL_MIX_MAXVOLUME);
        SDL_MixAudio(stream, _aSwrOutFrame->data[0] + _aSwrOutIdx, filLen, volumn);
        len -= filLen;
        stream += filLen;
        _aSwrOutIdx += filLen;
    }
...
int VideoPlayer::decodeAudio()
{
    ...
    int outSamples = av_rescale_rnd(
        _aSwrOutSpec.sampleRate, _aSwrInFrame->nb_samples, _aSwrInSpec.sampleRate, AV_ROUND_UP);
